...
function validateBinaryVersion(...command) {
  ...
  const currentVersionWithoutRevision = "1.46.0".split("-")[0];
  if (stdout !== currentVersionWithoutRevision) {
    throw new Error(
      `Expected ${JSON.stringify(
        currentVersionWithoutRevision
...
function installUsingNPM(pkg, subpath, binPath) {
  ...
  const libDir = path2.dirname(require.resolve("@bufbuild/buf"));
  const installDir = path2.join(libDir, "npm-install");
  fs.mkdirSync(installDir);
  try {
    fs.writeFileSync(path2.join(installDir, "package.json"), "{}");
    child_process.execSync(
      `npm install --loglevel=error --prefer-offline --no-audit --progress=false ${pkg}@${"1.46.0"}`,
